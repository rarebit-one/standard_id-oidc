...
module StandardId
  module Oidc
    module Generators
      # Installs StandardId::Oidc in a host Rails application.
      #
      ...
      class InstallGenerator < Rails::Generators::Base
        ...
        desc <<~DESC
          Installs StandardId::Oidc. By default this:
            * copies the engine's migrations into db/migrate/
            * writes config/initializers/standard_id_oidc.rb
            * mounts StandardId::Oidc::Engine in config/routes.rb
        ...
        class_option :skip_initializer, type: :boolean, default: false,
          desc: "Do not write config/initializers/standard_id_oidc.rb"
        class_option :skip_routes, type: :boolean, default: false,
          desc: "Do not mount StandardId::Oidc::Engine in config/routes.rb"
        class_option :mount_path, type: :string, default: "/",
          desc: "Path to mount StandardId::Oidc::Engine at"
        class_option :force, type: :boolean, default: false,
          desc: "Overwrite config/initializers/standard_id_oidc.rb if it already exists"

        # Copies the engine's migrations with the same `.standard_id_oidc`
        # suffix `rails standard_id_oidc:install:migrations` uses, so the
        ...
        def copy_migrations
          ...
          if existing_migrations.any?
            say_status(
              "identical",
              "StandardId::Oidc migrations already present (#{existing_migrations.size}), skipping",
              :blue
            )
            return
          end
          ...
          engine_migrations.each_with_index do |source, index|
            ...
            copy_file source, "db/migrate/#{migration_number(index)}_#{name}.standard_id_oidc.rb"
          end
        end

        def copy_initializer
          path = "config/initializers/standard_id_oidc.rb"
        ...
        def mount_engine
          ...
          route(%(mount StandardId::Oidc::Engine => "#{options[:mount_path]}"))
        end

        def print_next_steps
          say ""
          say "StandardId::Oidc installed. Next:", :green
